...
- [x] **색상 경고 시스템:** 사용량(CPU/GPU/Memory/Disk) 및 온도, 배터리 잔량에 따라 수치를 주황·빨강·초록으로 강조 표시.
- [x] **Fan 모니터링:** Apple Silicon SMC(IOKit)를 통해 Fan RPM 및 최대 RPM 추적. 팬리스 기기는 "팬 없음" 표시, RPM 비율에 따라 색상 경고 적용.
- [ ] **프로세스 상세 보기:** 현재 CPU 또는 메모리를 가장 많이 사용하는 상위 3개 프로세스를 보여주는 "Top Consumers" 서브 메뉴.

#### 🤖 AI 에코시스템 통합
- [x] **Ollama 모델 언로드 감지:** 모델 언로드 후 3~6초 내에 "로드된 모델 없음"으로 자동 갱신. VRAM/CPU 서브메뉴 잔상도 함께 초기화.
